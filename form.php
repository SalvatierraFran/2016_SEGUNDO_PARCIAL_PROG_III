...
//IMPLEMENTAR...
    ...
    if (isset($_POST["id"]))                     //Evita entrar desde el navegador con ruta directa
    {
        if ($_POST["id"] != 0)
...
<div id="divFrm" class="animated bounceInLeft" style="height:330px;overflow:auto;margin-top:0px;border-style:solid">
    ...
    <input type="text" placeholder="Nombre" id="txtNombre" value="<?php //IMPLEMENTAR...
        echo $unUser->nombre;
     ?>" <?php if($_POST["queHago"] == 'Eliminar') echo 'readonly'; ?> />
    ...
    <span>Perfil</span>
    <select id="cboPerfiles" <?php if(isset($_POST["registrar"]) || ($_POST["queHago"] == 'Eliminar' || $usuarioLogueado->perfil != "administrador")) echo "disabled"; ?>>
        <?php 
		//IMPLEMENTAR...
            $todosLosPerfiles = Usuario::TraerTodosLosPerfiles();
            foreach ($todosLosPerfiles as $value) {
                        # code...
    ...
    <input type="button" class="MiBotonUTN" onclick="
    
    <?php echo ($_POST["queHago"] != 'Editar') ? ($_POST["queHago"] . 'Usuario()') : 'ModificarUsuario()'; //IMPLEMENTAR... ?>" 

    value="<?php if(isset($_POST["registrar"])) echo "Registrarse"; else if ($_POST["queHago"] == 'Agregar') echo "Guardar"; else echo $_POST["queHago"]; //IMPLEMENTAR... ?>"  />
